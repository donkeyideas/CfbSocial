...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${lg.name} — College Football 26 Online Dynasty (${lg.platform}) | CFB Social`;
  const desc = `Join ${lg.name}, a ${styleLabel} ${lg.platform} College Football 26 online dynasty${lg.cross_play ? ' with cross-play' : ''}. ${open > 0 ? `${open} of ${lg.max_users} spots open.` : 'Currently full.'}${lg.open_schools ? ` Open schools: ${lg.open_schools}.` : ''}${lg.sim_schedule ? ` Sims ${lg.sim_schedule}.` : ''}`;
  return {
    title,
    description: desc,
    ...
    <div>
      ...
      <JsonLd
        data={{
          ...
          '@type': 'WebPage',
          name: `${lg.name} — College Football 26 Online Dynasty`,
          url: `https://www.cfbsocial.com/game-room/leagues/${id}`,
          about: { '@type': 'VideoGame', name: 'EA Sports College Football 26', alternateName: ['College Football 26', 'CFB 26'] },
      ...
      <div className="feed-header">
        ...
        <p style={{ fontFamily: 'var(--mono)', fontSize: '0.8rem', color: 'var(--faded-ink)', textAlign: 'center', marginTop: 4 }}>
          College Football 26 Online Dynasty · {lg.platform} · {styleLabel}
        </p>
      </div>

      <section className="content-card" style={{ marginBottom: 16 }}>
        <p style={{ fontFamily: 'var(--sans)', color: 'var(--ink)', lineHeight: 1.6 }}>
          <strong>{lg.name}</strong> is a {styleLabel.toLowerCase()} <strong>{lg.platform}</strong> EA Sports College Football 26 online dynasty
