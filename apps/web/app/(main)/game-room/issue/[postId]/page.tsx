...
async function load(postId: string) {
  ...
  const { getIssueByFeedPost } = await import('@cfb-social/api');
  const supabase = await createClient();
  return (await getIssueByFeedPost(supabase, postId).catch(() => null)) as
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = await load(postId);
  if (!data) return { title: 'Issue not found | CFB Social' };
  const { issue, items } = data;
  const masthead = issue.title || 'Game Room Weekly';
  const title = `${masthead} · Issue No. ${issue.issue_number} | CFB Social`;
  const desc = issue.cover_headline || `A College Football dynasty magazine issue from @${issue.owner?.username ?? 'a coach'} on CFB Social.`;
  ...
  return {
    ...
    alternates: { canonical: `https://www.cfbsocial.com/game-room/issue/${postId}` },
  };
}
